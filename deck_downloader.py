...
def hearthpwn_deck(deck_url):
	...
	deck_data = requests.get(deck_url).text										# get HTML
	deck_data = BeautifulSoup(deck_data, 'html.parser')							# parse with BeautifulSoup
	deck_data = deck_data.findAll("aside", { "class" : "infobox"})				# find infobox with deck cards
	deck_data = deck_data[0].findAll('a')										# find all links inside infobox
